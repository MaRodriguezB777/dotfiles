...
| [`subagents/`](extensions/subagents/) | Spawn, supervise and resume child pi sessions. Territory claims (glob write-scopes) are enforced inside each child so parallel agents cannot overwrite each other; children stand down when the parent dies and can be resumed later. `/subagents-fleet` is a live transcript inspector, `/subagents-info` dumps the exact prompts and token costs, `/subagents-resume-run` adopts a previous run. See its [README](extensions/subagents/README.md). |
